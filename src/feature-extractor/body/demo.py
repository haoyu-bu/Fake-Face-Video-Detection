...
def store_3d_pose(imname, detections, njts):
    result = str()
    for i, det in enumerate(detections):
        img_name = str(imname).split('/')[-1]
        fid = str(img_name).split('.')[0]
        result += str(fid) + ' '
        pose3d = det['pose3d']
        score = det['cumscore']
        
        for i in range(njts):
            result += str(pose3d[i]) + ' ' + str(pose3d[i+njts]) + ' ' + str(pose3d[i+2*njts]) + ' ' # (x, y, z)
        result += '\n'
    return result
...
def display_poses( image, detections, njts):
    ...
    for i,det in enumerate(detections):
        ...
        if njts==13: # other bones on torso for 13 jts
            ...
            ax.plot( x, y, z, 'b', scalex=None, scaley=None, lw=lw)
        # put red markers for all joints
        ax.plot( pose3d[0+10:njts-1], pose3d[njts+10:2*njts-1], pose3d[2*njts+10:3*njts-1], color='r', marker='.', linestyle = 'None', scalex=None, scaley=None)
        # score
...
def demo( imagelist, modelname, gpuid):
    ...
    for imagename in os.listdir(imagelist):
        imagename = os.path.join(imagelist, imagename)
        if not os.path.isfile(imagename):
            print("ERROR: Image {:s} does not exist".format(imagename))
            sys.exit(1)
        img_output_list.append((imagename, None))
    ...
    for i,(imname,_) in enumerate(img_output_list): # for each image
        ...
        results += store_3d_pose(imname, detections, njts)
    with open("processed/" + str(imagelist.split('/')[-2]) + "_results.txt", 'w') as f:
        f.write(results)
    

if __name__=="__main__":
    if len(sys.argv) not in [3,4]:
        print("Usage: python demo.py <modelname> <imagelist> [<gpuid>]")
        sys.exit(1)
    ...
    demo(imagelist, modelname, gpuid)
